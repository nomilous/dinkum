...
var EventEmitter, deferred, testable;
...
exports.Queue = function(config) {
  ...
  queue = {
    ...
    enqueue: deferred(function(action, object) {
      if (queue.pending.count === config.queueLimit) {
        return action.reject(new Error('dinkum queue overflow (use config.queueLimit)'));
      }
  ...
  api = {
    ...
    on: function(event, handler) {
      return queue.emitter.on(event, handler);
    }
  };
  return Object.defineProperty(api, 'suspend', {
    get: function() {
      return queue.suspended;
    },
    set: function(value) {
      return queue.suspended = value;
    }
  });
};
